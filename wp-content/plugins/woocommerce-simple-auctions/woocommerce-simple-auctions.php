			...
			public function wpml_notice() {
				...
				if ( current_user_can( 'manage_options' ) ) {
					...
					if ( ! get_user_meta( $user_id, 'auctions_wpml_ignore' ) ) {
						...
						<p>' . sprintf( __( 'WooCommerce Simple Auctions - multi currency feature not supported.  <a href="%1$s">Hide Notice</a>', 'wc_simple_auctions' ), esc_attr( add_query_arg( 'auctions_wpml_ignore', '0' ) ) ) . '</p>
						</div>';
					}
				}
			}
			...
			public function frontend_enqueue_script() {
				...
				$custom_data                         = array(
					...
					'outbid_message'  => wc_get_template_html( 'notices/error.php', 
						array( 
							'messages' => array( __( "You've been outbid!", 'wc_simple_auctions' ) 	) ,
							'notices'  => array_filter( array( 'error' => array( 'notice' => __( "You've been outbid!", 'wc_simple_auctions' ) ) ) ),
						)
					)
			...
			public function product_write_panel() {
				...
				$auction_dates_to   = ( $date = get_post_meta( $post->ID, '_auction_dates_to', true ) ) ? $date : '';
				 ...
				 echo '<p class="form-field auction_dates_fields">
						...
						<input type="text" class="short datetimepicker required" name="_auction_dates_from" id="_auction_dates_from" value="' . $auction_dates_from . '" placeholder="' . _x( 'From&hellip; YYYY-MM-DD HH:MM', 'placeholder', 'wc_simple_auctions' ) . '" maxlength="16" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])[ ](0[0-9]|1[0-9]|2[0-4]):(0[0-9]|1[0-9]|2[0-9]|3[0-9]|4[0-9]|5[0-9])"  />
						<input type="text" class="short datetimepicker required" name="_auction_dates_to" id="_auction_dates_to" value="' . $auction_dates_to . '" placeholder="' . _x( 'To&hellip; YYYY-MM-DD HH:MM', 'placeholder', 'wc_simple_auctions' ) . '" maxlength="16" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])[ ](0[0-9]|1[0-9]|2[0-4]):(0[0-9]|1[0-9]|2[0-9]|3[0-9]|4[0-9]|5[0-9])"  />
					</p>';
