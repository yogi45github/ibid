	...
	if( $product_data ->get_auction_sealed() === 'yes' ){
		...
		if($current_user->ID == $product_data->get_auction_current_bider()){
			if(!$product_data->is_reserve_met() && ('yes' !== $product_data->get_auction_sealed() ) ){
				$message = '<div class="reserve_not_meet" data-auction-id="' . $product_data -> get_id() . '" >' . sprintf( __( 'Successfully placed a bid for &quot;%s&quot; but it does not meet the reserve price!', 'wc_simple_auctions' ),$product_data -> get_title()  ) .'</div>' ;
			} else{

				if($product_data->get_auction_proxy() && $product_data->get_auction_max_bid()){
					$message = sprintf( __( 'Successfully placed a bid for &quot;%s&quot;! Your max bid is %s.', 'wc_simple_auctions' ),$product_data -> get_title(), wc_price($product_data->get_auction_max_bid())  );
				}else{
					$message = sprintf( __( 'Successfully placed a bid for &quot;%s&quot;!', 'wc_simple_auctions' ),$product_data -> get_title()  );
				}
			}	
