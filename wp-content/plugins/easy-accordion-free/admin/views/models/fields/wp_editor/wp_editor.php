...
if ( ! class_exists( 'SP_EAP_Field_wp_editor' ) ) {
  class SP_EAP_Field_wp_editor extends SP_EAP_Fields {
    ...
    public function render() {
      ...
      echo ( eapro_wp_editor_api() ) ? '<div class="eapro-wp-editor" data-editor-settings="'. esc_attr( json_encode( $editor_settings ) ) .'">' : '';
    ...
    public function setup_wp_editor_media_buttons() {
      ...
      echo 'var eapro_media_buttons = '. json_encode( $media_buttons ) .';';
      echo '</script>';
    ...
    public function setup_wp_editor_settings() {
      ...
      if ( eapro_wp_editor_api() && class_exists( '_WP_Editors') ) {
        ...
        $defaults = apply_filters( 'eapro_wp_editor', array(
          'tinymce' => array(
            'wp_skip_init' => true
          ),
        ) );
