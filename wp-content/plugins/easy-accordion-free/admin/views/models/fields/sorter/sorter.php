 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'SP_EAP_Field_sorter' ) ) {
	class SP_EAP_Field_sorter extends SP_EAP_Fields {
		...
		public function render() {
			...
			$args = wp_parse_args(
				$this->field, array(
					'disabled'       => true,
					'enabled_title'  => esc_html__( 'Enabled', 'easy-accordion-free' ),
					'disabled_title' => esc_html__( 'Disabled', 'easy-accordion-free' ),
				)
			);
			...
			echo ( $args['disabled'] ) ? '<div class="eapro-modules">' : '';

			echo ( ! empty( $args['enabled_title'] ) ) ? '<div class="eapro-sorter-title">' . esc_attr( $args['enabled_title'] ) . '</div>' : '';
			echo '<ul class="eapro-enabled">';
			...
			if ( $args['disabled'] ) {
				...
				echo '<div class="eapro-modules">';
				echo ( ! empty( $args['disabled_title'] ) ) ? '<div class="eapro-sorter-title">' . esc_attr( $args['disabled_title'] ) . '</div>' : '';
				echo '<ul class="eapro-disabled">';
