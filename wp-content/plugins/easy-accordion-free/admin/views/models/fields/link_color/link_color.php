 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'SP_EAP_Field_link_color' ) ) {
	class SP_EAP_Field_link_color extends SP_EAP_Fields {
		...
		public function render() {
			...
			$color_props = array(
				'color'   => esc_html__( 'Normal', 'easy-accordion-free' ),
				'hover'   => esc_html__( 'Hover', 'easy-accordion-free' ),
				'active'  => esc_html__( 'Active', 'easy-accordion-free' ),
				'visited' => esc_html__( 'Visited', 'easy-accordion-free' ),
				'focus'   => esc_html__( 'Focus', 'easy-accordion-free' ),
			);
			...
			foreach ( $color_props as $color_prop_key => $color_prop_value ) {
				...
				if ( ! empty( $args[ $color_prop_key ] ) ) {
					...
					echo '<div class="eapro--title">' . esc_attr( $color_prop_value ) . '</div>';
					echo '<input type="text" name="' . esc_attr( $this->field_name( '[' . $color_prop_key . ']' ) ) . '" value="' . esc_attr( $value[ $color_prop_key ] ) . '" class="eapro-color"' . $default_attr . $this->field_attributes() . '/>';
					echo '</div>';
